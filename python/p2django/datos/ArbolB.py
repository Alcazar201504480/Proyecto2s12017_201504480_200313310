import bisect
import copy
from datos.ArbolAvl import Avl, archivo
from datos.Cmd import cmd
...
class _Carpeta(object):
    ...
    def agregarArchivo(self, nombre, file):
        a=archivo(nombre)
        a.file = file
        self.archivos.agregar_objeto(a)
    ...
    def eliminarArchivo(self, nombre_actual):
        return self.archivos.borrar(nombre_actual)
    ...
    def buscarCarpetaPorPath(self, path):
        nombre_carpeta = ""
        carpeta_final = None
        print("buscar carpeta por path " + path)
        if path.find("/") == 0:
            path = path[1:]
        if path.find("/") == len(path)-1:
            print("remover slash final")
            path = path[:-1]
            print("path despues de " + path)
        if path.find("/") > 0:
        ...
        print ("carpeta encontrada = " + str(carpeta_final))
        print (carpeta_final.graficarArbolB())
...
class _NodoArbol(object):
    ...
    def obtenerListaCarpetas(self):
        listaCarpetas = copy.deepcopy(self.carpetas)
        print("largo de nodo = " + str(len(self.carpetas)))
        if self.hijos:
            for hijo in self.hijos:
                listaHijos = copy.deepcopy(hijo.obtenerListaCarpetas())
                listaCarpetas.extend(listaHijos)
